...
<div class="sm:container sm:mx-auto mx-5 mt-5 flex flex-col justify-center gap-5">
    <h1 class="text-xl w-full mx-auto text-center sm:text-3xl">Create an account</h1>
        ...
        <form class="w-full mx-auto" method="POST">
            <div class="flex flex-col gap-5 w-full">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="password" name="passwordConfirm" placeholder="password" class="input input-bordered w-full" bind:value={$form.passwordConfirm} />
                    {#if $errors.passwordConfirm}
                        <p class="text-xs text-red-600">{$errors.passwordConfirm}</p>
                    {/if}
                    {#if $message?.passwordConfirm}
                        <p class="text-xs text-red-600">{$message.passwordConfirm.message}</p>
                    {/if}
                </div>
                <p class="text-sm">Already have an account? <a href="/auth/login" class="link">Log in</a></p>
